// all the functions related to Atman update, including showing up modal, notification etc will be here
...
function check_rengine_update() {
  if (
    ...
    Swal.fire({
      title: "Checking Atman latest version...",
      ...
      .then(function (response) {
        ...
        if (response["description"] == "RateLimited") {
          ...
          Swal.fire({
            ...
            text: "You are running the latest version of Atman!",
            icon: "info",
          });
        }
      });
...
function update_available(latest_version_number, changelog) {
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      html: `
          ${changelogStyle}
          <h5>Atman's new update ${latest_version_number} is available, please follow the update instructions.</h5>
...
function showAfterUpdatePopup() {
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // Swal.fire({
    //   title: isFirstRun ? "Welcome to Atman!" : "Thanks for updating!",
